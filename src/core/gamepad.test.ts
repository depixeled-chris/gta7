...
describe('readGamepadIntent', () => {
  ...
  it('left stick forward is +y (axis up is -1)', () => {
    const r = readGamepadIntent([0, -1, 0, 0], noButtons);
    expect(r.y).toBeCloseTo(1, 6);
    expect(r.x).toBe(0);
  });

  it('right trigger accelerates, left trigger reverses', () => {
    const buttons = noButtons.slice();
    buttons[GP.RT] = 1;
    expect(readGamepadIntent([0, 0, 0, 0], buttons).y).toBeCloseTo(1, 6);
    buttons[GP.RT] = 0;
    buttons[GP.LT] = 1;
    expect(readGamepadIntent([0, 0, 0, 0], buttons).y).toBeCloseTo(-1, 6);
  });

  it('stick + trigger sum but stay clamped to [-1,1]', () => {
    const buttons = noButtons.slice();
    buttons[GP.RT] = 1;
    const r = readGamepadIntent([0, -1, 0, 0], buttons); // stick fwd + full RT
    expect(r.y).toBe(1);
  });
